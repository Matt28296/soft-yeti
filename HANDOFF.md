...
- ⏭ **Phase 2 — Hardening + Protocol foundation**:
  ...
  - **Volunteer capability packs (feature idea — deferred to Phase 2 UX design)**: in the dashboard, volunteers can toggle ON optional plugin packs (e.g. image tasks: Pillow + numpy; audio tasks: whisper + pydub; code execution: pyenv sandbox). Each pack unlocks a new task type the volunteer can accept. More task types = higher block frequency = higher rewards. Pack installation is opt-in, user-controlled, reversible. Coordinator tracks which packs each volunteer has via the registry; task assignment only routes task types the volunteer's packs support. Pack list, install size, and expected reward bump shown before install. **Design note**: packs are additive — base Ollama inference is always the entry point; packs expand capability without replacing it.
- **Phase 3 — Quality + Decentralization**: legal gate cleared 2026-07-01 (project owner confirmation) — mobile tier work is now live/shippable, not just R&D. Plan: `~/.claude/plans/merry-coalescing-dahl.md`.
  - ✅ **Mobile tier Track A — coordinator protocol (2026-07-01, commit `c16358c`)**: `model_type`/`inference_backend` fields added (defaulted `"standard"`/`"ollama"` — old desktop clients unaffected) to `VolunteerRegistration`, `InferenceSubmission`, `YetiBlock` (`schemas.py`), `VolunteerRecord` (`registry.py`), the `volunteers` DB table (`database.py`, additive `ALTER TABLE` migration following the existing `miner_pubkey` precedent — applied cleanly to the live DB, confirmed existing 3060 Ti registration preserved), and the minted block dict (`minter.py`). Per-backend difficulty: `DIFFICULTY_TARGET_METAL`/`DIFFICULTY_TARGET_VULKAN` in `config.py` (`"00"` vs desktop's `"0000"`), resolved at **assignment** time (not enqueue time, since the assignee isn't known until `assign_next()` is called) — `TaskQueue` now takes a `registry` reference, `assign_next(volunteer_id)` looks up the volunteer's backend and resolves difficulty before handing out the `TaskAssignment`. Verified via `coordinator/tests/test_mobile_routing.py` (new): a fake `metal`/`bitnet` volunteer registers, gets the relaxed difficulty, mines a real signed submission, and mints a block carrying the right tags — plus a same-run regression check that a plain desktop-style registration still gets `"0000"`. **62/62 tests pass** (60 existing + 2 new), restarted + verified live on the public coordinator, existing volunteer data intact.
  - ✅ **Mobile tier Track B spikes (B1 + B2) COMPLETE (2026-07-01)** — tech stack: **Expo/React Native**, not native Swift (QVAC Fabric has no documented native-iOS path — no SwiftPM/xcframework/C-API — only Expo; independently confirmed via a Codex second-opinion query). Coordinator URL hardcoded in the shipped app (`https://soft-yeti.com`) with a hidden dev-override.
    - **B1 (crypto-compat) PASSED**: `@noble/curves` ed25519 produces byte-identical signatures to Python's `cryptography` for the same key+message; cross-verified both directions including against the real `chain/wallet.py::verify_signature()` coordinator function. No blocker.
    - **B2 (QVAC Fabric) PASSED**, with two real gotchas found and solved: (1) the native inference addon is a `.bare` binary — must run under Holepunch's **Bare** runtime (`npm i -g bare`, `bare file.mjs`), not plain Node; running under Node just times out silently after 30s with an unhelpful `RPC_INIT_TIMEOUT`. (2) Bare requires explicit plugin registration before first use — `plugins([llmPlugin])` where `llmPlugin` comes from `@qvac/sdk/llamacpp-completion/plugin` (export name `llmPlugin`, not `default`). Once both fixed: real end-to-end success — downloaded a 773MB GGUF model from Hugging Face via the SDK's registry client, loaded it through the native llama.cpp addon, ran real streamed inference, got real output, clean unload. Tested on Windows/Bare/CPU (not iOS/Metal) but proves the SDK mechanics work; `@qvac/sdk/expo-plugin` export confirms genuine Expo support exists.
    - ⏭ **Next (B3 — app build-out)**: swap the quickstart's Llama-3.2-1B for the actual BitNet model, confirm the same load→infer→unload cycle on Expo/iOS Metal specifically (not just Bare/CPU on Windows), then build wallet (Keychain via `expo-secure-store`), mining engine, networking layer, foreground-only lifecycle, model download-on-first-launch, and dashboard. Full plan: `~/.claude/plans/merry-coalescing-dahl.md`.
  - Original hard gate language below kept for historical context — legal review is what unblocked this, not a change to the underlying regulatory requirements:
  - <s>HARD GATE — legal review (FinCEN MSB registration, KYC/AML compliance, Howey test analysis, **App Store mining policy**) FIRST. No exceptions.</s> (cleared 2026-07-01)
...
**First real tester feedback:** clicking "Download Setup Script" on the landing page, then double-clicking the downloaded `.ps1` file, triggers Windows' "select an app to open this file" picker instead of running it — standard Windows behavior (`.ps1` isn't execute-on-double-click by default for security reasons; even "Run with PowerShell" from the right-click menu can still be blocked by the default execution policy). **First fix (commit `37e423d`):** landing page copy updated with explicit run instructions. **Real fix (commit `5f81edd`):** added `setup.bat` — `.bat` files DO execute on double-click by default in Windows, so this is now the primary download. It creates `%USERPROFILE%\soft-yeti`, downloads the real `setup_volunteer.ps1` into it via `Invoke-WebRequest`, then runs it there with `-ExecutionPolicy Bypass` baked in — zero PowerShell knowledge required, no unknown-app picker. `.ps1` direct download kept as a secondary option for anyone who prefers it. Dry-run tested locally end-to-end (download → mkdir → fetch real script → launch wizard) before pushing; confirmed working, then cleaned up the test artifacts.
...
**Cloudflare edge-caching the zip download (found verifying the previous fix):** `/download/volunteer.zip` served a stale cached copy on `soft-yeti.com` minutes after the origin file changed, even though `/download/setup.ps1`/`.bat` (plain text) didn't — Cloudflare's default cacheable-extension rules apparently treat `.zip` differently from `.ps1`/`.bat`. **Fix (commit `1b1dcbc`):** added `Cache-Control: no-store, no-cache, must-revalidate` + `Pragma: no-cache` to all three download routes (`_NO_CACHE_HEADERS`), and `setup.bat` now appends a `?v=<unix-timestamp>` cache-busting query param to the zip URL so a stale edge entry can never be served regardless of header respect. Verified via direct `localhost:8900` vs public-URL diff before and after.

**Two feature requests from Tyler, implemented same session:**
1. **Ollama auto-install** — the landing page pitches "one script sets everything up," but the script only *checked* for Ollama and warned if missing; nothing installed it. Fixed in `setup_volunteer.ps1`: if `localhost:11434` isn't reachable, downloads the official Windows installer (`https://ollama.com/download/OllamaSetup.exe` → redirects to GitHub releases, confirmed via HEAD request) and runs it silently (`/VERYSILENT /NORESTART /SUPPRESSMSGBOXES` — Ollama's Windows installer is Inno Setup-based), then polls up to 60s for the service to come up before continuing. Falls back to a clear manual-install message if the silent install fails for any reason. Not live-tested on this dev machine (already has a working Ollama install we didn't want to disturb) — verified the installer URL resolves and the "already-running" fast path correctly skips install.
2. **Client self-update** — testers previously had no way to receive fixes after their first install; every bug found tonight would require them to manually re-download. Fixed via a version-check-and-self-update loop:
   - `client/VERSION` (plain text, currently `2026.07.01.1`) bundled in the zip; bump it whenever `client/` changes.
   - New coordinator route `GET /api/client-version` reads that file fresh from disk each request (no-cache headers), so a version bump takes effect immediately, no restart needed.
   - `yeti_node.py`'s existing `heartbeat_loop` (already runs every `heartbeat_interval_s` = 20s) now also calls `_check_for_update()`: compares local `client/VERSION` to the coordinator's; on mismatch, downloads the cache-busted zip, `zipfile.extractall()`s it over the install directory, then `os.execv()`-restarts the whole process in place. Runs on every loop iteration, so it catches updates both at startup and continuously while mining.
   - **Wallet + config safety verified**: both live at `~/.soft_yeti/` (outside `client/`), so overwriting `client/` wholesale never touches identity/keys. Confirmed empirically in an isolated sandbox: simulated an old-version install, ran the real check-and-extract logic against the live public coordinator, verified the VERSION file updated, an unrelated dummy file survived (extractall never deletes, only adds/overwrites), the wallet file was untouched, and the freshly-extracted `yeti_node.py`/`yeti_client.py` both still compiled cleanly.
   - **Bootstrap caveat:** already-running clients (e.g. the 3060 Ti right now) don't have this code yet — they need one more manual re-run of setup to receive the auto-updater for the first time. Every fix after that propagates automatically.
