...
- **Phase 3 — Quality + Decentralization**: legal gate cleared 2026-07-01 (project owner confirmation) — mobile tier work is now live/shippable, not just R&D. Plan: `~/.claude/plans/merry-coalescing-dahl.md`.
  ...
  - ⏭ **Mobile tier Track B — iOS app**: tech stack decided as **Expo/React Native**, not native Swift — QVAC Fabric (the intended BitNet inference SDK) has no documented native-iOS integration path (no SwiftPM/xcframework/C-API), only a documented Expo path. Independently confirmed via a second-opinion Codex query. Coordinator URL hardcoded in the shipped app (`https://soft-yeti.com`) with a hidden dev-override, not a user-facing Settings field. See the plan doc for the full Track B sequencing (crypto-compat spike → QVAC spike → app build-out → real-device end-to-end run → TestFlight).
