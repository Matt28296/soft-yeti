...
```
block_gross = completion_tokens × 0.001 × nonce_attempts
base_gross  = total_completion_tokens × 0.0001
gross       = block_gross + base_gross
miner_share = gross × 0.90
treasury_share = gross × 0.10
```

`total_completion_tokens` is the sum of tokens across ALL nonce attempts. For old clients that don't send it, the coordinator falls back to `completion_tokens × nonce_attempts` (same value when each attempt produces equal tokens).
...
| **1 — Internal testers** | ⏳ Final gate | Canary temp fix ✅, output sanitization ✅, nonce_attempts ge=1 ✅, VRAM auto-detect + model ladder ✅, exponential backoff ✅, `normalize_canary_output()` ✅, canary **50/50 on 7B** ✅ (`860b39c`+`5d88680`), PWA dashboard at localhost:8901 ✅ (`75a0116`+`bdf4557`), GPU name persisted in config ✅ (`64fb1c1`), **named tunnel live at soft-yeti.com** ✅, canary **50/50 on 1.5B** (laptop testers) ✅ (`ff02d5e`). **60/60 tests pass.** Remaining: 5-tester invite. |
| **1.5 — Zero-protocol quality wins** | ✅ Complete | Quality gate in client (`_passes_quality_gate` — filters <20-char and repetition-looping outputs before hash check). Best-output delivery (`best_output()` in sanitizer scores all nonce-attempt outputs by length+repetition; coordinator delivers highest-quality to J-Claw, not just the hash-winner). Base rate reward (`total_completion_tokens × 0.0001` added to block gross; new block fields `total_completion_tokens` + `base_reward`). Old clients unaffected (all new fields optional with defaults). Commit `8feb6e0`. **60/60 tests pass.** |
| **2 — Hardening + Protocol foundation** | ⏭ | Argon2 memory-hard PoW (256MB/attempt). Vulkan/PyOpenCL real GPU benchmark (backend-agnostic interface designed for mobile Metal in Phase 3). True model fingerprinting (per-model canary calibration). **Proof of Inference Depth (PoID)** — N self-refinement passes cryptographically chained; all passes delivered; reward ∝ depth; eliminates hash lottery. On-chain model registry (weights hash, reputation score, family — `model_type`/`inference_backend` fields ✅ already shipped ahead of schedule, see Phase 3 row). Difficulty auto-adjustment. Subscription economy live. `SoftYetiSetup.exe` one-click installer. |
| **3 — Quality + Decentralization** | ⏳ In progress | Legal gate **cleared 2026-07-01**. **Mobile tier Track A (coordinator protocol) ✅ complete** (commit `c16358c`): `model_type: standard\|bitnet` + `inference_backend: ollama\|metal\|vulkan` fields added everywhere (registration/submission/blocks, all defaulted — old desktop clients unaffected), per-backend difficulty resolved at task-assignment time (mobile `"00"` vs desktop `"0000"`), additive DB migration applied live with zero data loss, verified via `test_mobile_routing.py` (fake mobile volunteer end-to-end + desktop regression check). **62/62 tests pass.** ⏭ **Mobile tier Track B (iOS app)**: tech stack is **Expo/React Native**, not native Swift — QVAC Fabric has no documented native-iOS path (confirmed via direct research + independent Codex second opinion), only Expo. Coordinator URL hardcoded in the shipped app. Plan: `~/.claude/plans/merry-coalescing-dahl.md`. ⏭ Still gated behind Track A/B: BFT multi-validator consensus (5-of-9 threshold sigs, staked YETI, slashing). Reference model judge. Peer quality committee. Bayesian miner reputation (churn-tolerant for mobile). Model diversity enforcement. Elastic emission + 30% fee burn. DHT task queue. |
